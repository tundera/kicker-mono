...
export class AppSyncCdkStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const lambdaExecutionRole = new Role(this, `${process.env.SERVICES_PREFIX}_FuncExecutionRole`, {
      ...
      bundling: {
        ...
        commandHooks: {
          beforeBundling(inputDir: string, outputDir: string): string[] {
            const schemaPath = path.join(inputDir, process.env.PRISMA_SCHEMA_ROOT_PATH || 'schema.prisma')
    ...
    const dataSource_lambda = new LambdaDataSource(this, `${process.env.SERVICES_PREFIX}LambdaDatasource`, {
      api: graphqlApi,
    ...
    const dataSource_none = new NoneDataSource(this, `${process.env.SERVICES_PREFIX}NoneDatasource`, {
      api: graphqlApi,
    ...
    const resolvers = yaml.safeLoad(
      fs.readFileSync(path.join(__dirname, process.env.APPSYNC_RESOLVERS_PATH || ''), 'utf8'),
    )
